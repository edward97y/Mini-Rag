...
class ChunkModel(BaseDataModel):
    ...
    async def init_instance(self):
        ...
        if DataBaseEnum.COLLECTION_CHUNK_NAME.value not in all_collection:
            self.collection = self.db_client[DataBaseEnum.COLLECTION_CHUNK_NAME.value]
            indexes=DataChunk.get_chunk_indexes()
    ...
    async def get_chunk_by_project_id(self,project_id:ObjectId,page_no:int=1,page_size:int=50):
